  ...
  return (
    <div className="flex items-center justify-center">
      <motion.div ref={cardRef} className={`relative w-[18.5rem] h-[21.2rem] rounded-[15px] overflow-hidden shadow-lg dark:shadow-[0_4px_10px_rgba(255,255,255,0.1)] bg-neutral-50 dark:bg-black ${spotlight && isRevealed ? 'group' : ''}`} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={isRevealed ? { rotateX, rotateY, translateX, translateY, transformStyle: "preserve-3d", transition: "transform 0.1s ease-out" } : {}}>
        ...
        <motion.div initial={{ width: "100%", height: "100%" }} animate={{ width: isImageShrunken ? "6rem" : "100%", height: isImageShrunken ? "7rem" : "100%", top: isImageShrunken ? "4.5rem" : 0, left: isImageShrunken ? "1rem" : 0, borderRadius: isImageShrunken ? "0.5rem" : "0px" }} transition={{ duration: 0.4, ease: "easeInOut" }} className="absolute overflow-hidden">
                ...
                {socialLinks.map((social, index) => {
                  const renderIcon = () => {
                    if (typeof social.icon === 'string') {
                      return <Image src={social.icon} alt={social.name} width={20} height={20} className="object-contain" />;
