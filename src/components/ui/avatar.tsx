...
>(({ className, variant = "none", ...props }, ref) => {
  ...
  return variant === "none" ? (
    <AvatarPrimitive.Root
      ref={ref}
      className={cn(
        "relative flex h-14 w-14 shrink-0 overflow-hidden rounded-[45%]",
        className
      )}
      {...props}
    />
    ...
    <div
      className={cn(
        "h-16 w-16 rounded-[45%] p-[2px]",
        ringClass,
        "flex items-center justify-center"
      )}
    >
      <div className="h-full w-full rounded-[45%] bg-black flex items-center justify-center overflow-hidden">
        <AvatarPrimitive.Root
          ref={ref}
          className={cn(
            "h-full w-full rounded-[45%] overflow-hidden",
            className
          )}
          {...props}
        />
...
>(({ className, ...props }, ref) => (
  <AvatarPrimitive.Fallback
    ref={ref}
    className={cn(
      "flex h-full w-full items-center justify-center rounded-[45%] bg-muted text-sm font-medium",
      className
    )}
    {...props}
  />
